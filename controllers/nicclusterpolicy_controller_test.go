	...
	Context("when a NicClusterPolicy with nv-ipam is created", func() {
		It("sets the NVIPAMReady per-component condition and the aggregate conditions", func() {
			...
			Eventually(func(g Gomega) {
				...
				conds := found.Status.Conditions
				g.Expect(apimeta.FindStatusCondition(conds, mellanoxv1alpha1.ConditionTypeReady)).NotTo(BeNil())
				g.Expect(apimeta.FindStatusCondition(conds, mellanoxv1alpha1.ConditionTypeProgressing)).NotTo(BeNil())
				g.Expect(apimeta.FindStatusCondition(conds, mellanoxv1alpha1.ConditionTypeDegraded)).NotTo(BeNil())
			}, timeout*3, interval).Should(Succeed())
