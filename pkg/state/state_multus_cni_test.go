...
var _ = Describe("Multus CNI state", func() {
	...
	It("should render ConfigMap with custom config for thin mode when config is specified in CR", func() {
		cr := getMinimalNicClusterPolicyWithMultus()
		...
		Expect(runFuncForObjectInSlice(objs, "ConfigMap", func(obj *unstructured.Unstructured) {
			...
			Expect(configMap.Name).To(Equal("multus-cni-config"))
		...
		Expect(runFuncForObjectInSlice(objs, "DaemonSet", func(obj *unstructured.Unstructured) {
			...
			Expect(mainContainer.Args).To(ContainElement("--multus-conf-file=/tmp/multus-conf/00-multus.conf"))
			Expect(mainContainer.VolumeMounts).To(ContainElement(
				corev1.VolumeMount{Name: "multus-cni-config", MountPath: "/tmp/multus-conf"},
			))
				...
				corev1.Volume{
					...
					VolumeSource: corev1.VolumeSource{
						ConfigMap: &corev1.ConfigMapVolumeSource{
							LocalObjectReference: corev1.LocalObjectReference{Name: "multus-cni-config"},
							Items: []corev1.KeyToPath{
								{Key: "cni-conf.json", Path: "00-multus.conf"},
							},
						},
					},
				},
	...
	It("should render thick DaemonSet with correct structure when DeploymentType is thick", func() {
		...
		Expect(runFuncForObjectInSlice(objs, "DaemonSet", func(obj *unstructured.Unstructured) {
			...
			// hostPID is required for the thick daemon to resolve delegate pods' netns via /proc
			Expect(daemonSet.Spec.Template.Spec.HostPID).To(BeTrue())

			// Thick images only ship binaries under /usr/src/multus-cni/bin (no root symlinks)
			initContainers := daemonSet.Spec.Template.Spec.InitContainers
			Expect(initContainers).To(HaveLen(1))
			Expect(initContainers[0].Command).To(ContainElement("/usr/src/multus-cni/bin/install_multus"))
