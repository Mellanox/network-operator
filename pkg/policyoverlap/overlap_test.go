...
func TestDetectNodeOverlap_EmptySelector(t *testing.T) {
	...
	policies := []mellanoxv1alpha1.NicNodePolicy{
		makeNicNodePolicy("policy-all", map[string]string{}),          // empty selector matches all
		makeNicNodePolicy("policy-gpu", map[string]string{"role": "gpu"}), // matches only node-a
	}
...
func TestFormatNodeOverlap(t *testing.T) {
	...
	expected := `node selector overlap detected: node "node-a" is selected by policies: [policy-1, policy-2]; node "node-b" is selected by policies: [policy-2, policy-3]`
	if result != expected {
