...
type MultusSpec struct {
	...
	// "thin" (default): Multus runs as an in-process CNI plugin. The config field, if set,
	// provides the Multus CNI configuration file content.
	// "thick": Multus runs as a privileged daemon; CNI calls are proxied via a Unix socket shim.
	// The config field, if set, configures the daemon (daemon-config.json).
	// +optional
